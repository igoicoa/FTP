...
from utils import assembly_packet, extract_packet
from constants import BUF_SIZE, DATA_SIZE, MSG_DOWNLOAD, MSG_DOWNLOAD_REQ_ACK, TIMEOUT, MSG_INIT, MSG_UPLOAD_REQ, MSG_DOWNLOAD_REQ, MSG_UPLOAD, MSG_UPLOAD_ACK, MSG_CLOSE, MSG_EMPTY, MSG_INITACK, MSG_UPLOAD_REQ_ACK, MSG_FILE_NOT_FOUND, MSG_UPLOAD_ERROR
...
class ClientUDP():
    ...
    def generate_upload_package(self, filename, data):
        filesize = os.path.getsize(filename)
        upload_filename = os.path.basename(filename)
        print(f"[CLIENT] - Sending SEQ NUM: {self.sequence_number} ")
        ...
        while True:
            if(filesize == 0):
                ...
                try:
                    ...
                    packet_recv = extract_packet(data)
                    print(f"[CLIENT] - response: {vars(packet_recv)}")
                    while (packet_recv.mode == MSG_UPLOAD_ACK) and (packet_recv.sequence_number > self.base) and (self.window):
