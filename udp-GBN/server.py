...
from utils import assembly_packet, extract_packet, create_file, file_exists, write_into_file, format_address
from constants import BUF_SIZE, MSG_DOWNLOAD_REQ, TIMEOUTS, TIMEOUT, MSG_INIT, MSG_INITACK, MSG_UPLOAD_REQ, MSG_UPLOAD, MSG_UPLOAD_ACK, MSG_CLOSE, MSG_CLOSE_ACK, MSG_EMPTY, MSG_DOWNLOAD_REQ_ACK, MSG_FILE_NOT_EXISTS, MSG_ERROR, MSG_UPLOAD_REQ_ACK
...
class ClientStates(object):
    ...
    def add_new_client(self, client_addr, packet_sent, packet_recv):
        self.lock.acquire()
        try:
            self.states[f"{client_addr[0]}:{client_addr[1]}"] = {"sent": packet_sent, "recv": packet_recv, "last_sent": datetime.now(), 'timeouts': 0, 'handshake_completed': False, 'window':[], 'base': 1, 'nextSeqNum': 1}
        finally:
            self.lock.release()
...
class ServerWorker(Thread):
    ...
    def __create_upload_ack_packet(self, packet, last_packet_sent):
        return UDPPacket(MSG_UPLOAD_ACK, packet.filename, packet.length, last_packet_sent.sequence_number + 1)
